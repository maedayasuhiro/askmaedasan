...
export default function HomePage({ posts }) {
  return (
    <div
      className="min-h-screen bg-cover bg-center text-gray-900 p-4 md:p-8 font-serif"
      style={{ backgroundImage: "url('/bg-japan.jpg')" }}
    >
      <header className="sticky top-0 z-50 bg-white/70 backdrop-blur-md py-4 shadow-md w-full">
        <h1 className="text-2xl md:text-4xl font-bold text-center text-indigo-900">
      ...
      <p className="text-md md:text-lg mb-6 text-center max-w-xl">
        ...
        <strong className="text-red-700">Local Japanese staff will fulfill your request directly in Japan.</strong>
      </p>
      ...
      <section className="mb-10 max-w-3xl w-full">
        ...
        <ol className="list-decimal list-inside bg-white/90 p-6 rounded-xl shadow-md text-sm md:text-base">
      ...
      <section className="max-w-3xl w-full mb-10">
        ...
        <div className="space-y-6 text-sm md:text-base">
          <div className="bg-white/90 p-4 rounded shadow-md">
            <p className="mb-2">★★★★★</p>
      ...
      <section className="max-w-3xl w-full mb-10">
        ...
        {posts.map((post) => (
          <div key={post.slug} className="bg-white/90 p-4 rounded shadow-md mb-4">
            <h3 className="text-lg font-semibold">{post.title}</h3>
            <p className="text-gray-500 text-sm">{post.date}</p>
            <p>{post.excerpt}</p>
            <Link href={`/blog/${post.slug}`} className="text-red-600 underline">Read More</Link>
          </div>
        ))}
      </section>
      ...
      <footer className="mt-10 text-gray-700 text-center text-sm">© 2025 Ask Maeda-san. Making Japan Accessible.</footer>
    </div>
  );
}
